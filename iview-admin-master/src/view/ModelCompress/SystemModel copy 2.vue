    ...
    <div>
      ...
      <card shadow>
        <Row>
          ...
          <Col span="2">
            <Select
              v-if="modelsClassDataset.length > 0"
              style="width: 100px"
              v-model="selectedmodelsClassDataset"
            >
              <Option
                v-for="(item_modelname, index) in modelsClassDataset"
                :key="index"
                :value="item_modelname"
                >{{ item_modelname }}</Option
              >
            </Select>
      ...
      <card shadow>
        <Row>
          <!-- 数据集以及分类 -->
          <compress-images
            v-bind:selected-value-class="selectedValueClass"
            v-bind:selected-value-dataset="selectedValueDataSet"
            v-on:update-selected-value-class="selectedValueClass = $event"
            v-on:update-selected-value-dataset="selectedValueDataSet = $event"
          >
          </compress-images>
        </Row>
      </card>
      ...
      <card>
        ...
        <Row>
          <Button type="primary" style="margin-left: 45%">
            <a
              :href="'download-model/?model=' + CDCompressModelStatus.ModelName"
              style="color: white"
              >下载模型
            </a>
          </Button>
